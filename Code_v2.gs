...
// ── MISSIONS ─────────────────────────────────────────────────
const MISSIONS_HEADERS = ["Année","ID","Client","Type","Nom mission","Date début","Date fin","Jours","Forfait HT (€)","Frais mission (€)","CA net frais (€)","Lieu","Horaires","Statut","N° Devis lié","N° Facture liée","Ajouté au calendrier","Commentaires JSON"];
...
function lireMissions(annee) {
  ...
  for (let i = 2; i < data.length; i++) {
    ...
    if (annee && String(data[i][0]) !== String(annee)) continue;
    const o = {};
...
function ajouterMission(m) {
  ...
  sheet.appendRow([
    annee, id,
    m.client || "", m.type || "", m.nom_mission || "",
    m.date_debut || "", m.date_fin || "",
    parseFloat(m.jours) || 0,
    forfait, frais, ca_net,
    m.lieu || "", m.horaires || "",
    m.statut || "Demande",
    m.num_devis || "", m.num_facture || "",
    "Non", "[]"
  ]);
  return { succes: true, id };
}
...
function modifierMission(ligne, m) {
  ...
  const frais = parseFloat(m.frais_mission) || 0;
  const ca_net = Math.round((forfait - frais) * 100) / 100;
  // Col indices (1-based): 3:Client 4:Type 5:Nom 6:DateDébut 7:DateFin
  // 8:Jours 9:Forfait 10:Frais 11:CA net 12:Lieu 13:Horaires 14:Statut 15:Devis 16:Facture
  [
    [3, m.client], [4, m.type], [5, m.nom_mission],
    [6, m.date_debut], [7, m.date_fin],
    [8, parseFloat(m.jours) || 0],
    [9, forfait], [10, frais], [11, ca_net],
    [12, m.lieu], [13, m.horaires], [14, m.statut],
    [15, m.num_devis], [16, m.num_facture]
...
function ajouterCommentaireMission(ligne, commentaire) {
  const sheet = getMissionsSheet();
  const raw = sheet.getRange(ligne, 18).getValue() || "[]";
  let hist = [];
  ...
  sheet.getRange(ligne, 18).setValue(JSON.stringify(hist));
  return { succes: true };
}

function ajouterAuCalendrier(ligne) {
  const sheet = getMissionsSheet();
  const r = sheet.getRange(ligne, 1, 1, 18).getValues()[0];
  // 0:Année 1:ID 2:Client 3:Type 4:Nom 5:DateDébut 6:DateFin 7:Jours
  // 8:Forfait 9:Frais 10:CA net 11:Lieu 12:Horaires 13:Statut ...
  const type = String(r[3] || ""), nom = String(r[4] || ""), client = String(r[2] || "");
  const lieu = String(r[11] || ""), horaires = String(r[12] || "");
  const forfait = r[8] || 0;
  ...
  const dateDebut = parseSheetDate(r[5]);
  const dateFin = parseSheetDate(r[6] || r[5]);
  ...
  sheet.getRange(ligne, 17).setValue("Oui");
  return { succes: true };
}
